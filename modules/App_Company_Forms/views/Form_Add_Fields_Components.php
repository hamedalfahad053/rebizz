	...
	<div class="container-fluid">
		...
		<div class="card card-custom">
					...
					<div class="card-body">
						...
						<form class="form" id="FormAddFields" action="<?= base_url(APP_NAMESPACE_URL.'/Forms/Create_Fields_Components') ?>" method="post">
							<?= CSFT_Form() ?>
							<input type="hidden"  name="Form_id"       value="<?= $Query_Forms->Forms_id ?>" />
							<input type="hidden"  name="Components_id" value="<?= $Query_Components->components_id ?>" />

							<div class="form-group row">
								<div class="col-lg-4 mt-5">
									<label><?= lang('Type_Fields') ?></label>
									<select name="Fields_Add" id="Fields_Add"  class="form-control selectpicker" title="الرجاء إختيار نوع الحقل">
										<?php
										foreach ($Fields_data AS $key)
										{
											echo '<option value="'.$key['Fields_id'].'">'.$key['Fields_title'].'</option>';
										}
										?>
									</select>
								</div>
