...
class App_Report_Transaction extends Apps
{
    ...
    public function Report_HTML()
    {
        ...
        $Get_All_Preview_Transaction = app()->db->where('company_id', app()->aauth->get_user()->company_id);
        $Get_All_Preview_Transaction = app()->db->where('Transaction_id', $Get_Transactions->transaction_id);
        $Get_All_Preview_Transaction = app()->db->where('Transaction_id', $Get_Transactions->transaction_id);
        $Get_All_Preview_Transaction = app()->db->get('protal_transaction_preview_data');

        foreach ($Get_All_Preview_Transaction->result() as $R_P) {

            $Fields_Components_P = Query_Fields_Components(
                array("Forms_id"      => $R_P->Forms_id,
                    "Components_id"   => $R_P->Components_id,
                    "Fields_key"      => $R_P->data_key)
            )->row();

            if ($Fields_Components_P->Fields_Type == 'Fields') {

                $data_array_Preview_Transaction_A['{Value_TD_' . $R_P->data_key . '}'] = $R_P->data_value;

            } elseif ($Fields_Components_P->Fields_Type == 'List') {

                $data_q    = Transaction_data_by_key($Get_Transactions->transaction_id, $R_P->Forms_id, $R_P->Components_id, $R_P->data_key);
                $data_list = get_data_options_List_view($Fields_Components_P->Fields_id, $data_q);
                $data_array_Preview_Transaction_B['{Value_TD_' . $R_P->data_key . '}'] = $data_list;
        ...
        $array_mirge = array_merge($data_array_Preview_Transaction_A, $data_array_Preview_Transaction_B);
