        ...
        foreach ($Transaction_data AS  $Row)
        {
            ...
            <tr>
                <td class="text-center">
	                ...
	                echo Get_Client_Logo($LoginUser_Company_Path_Folder,$this->aauth->get_user()->company_id,Transaction_data_by_key($Row['transaction_id'],'LIST_CLIENT'));
	                ?>"  height="35" width="35" >
	                <br>
	                <?= date('Ymd',$Row['Create_Transaction_Date']).$Row['transaction_id'];?>
                </td>
                <td class="text-center">
	                المالك : <?= Transaction_data_by_key($Row['transaction_id'],'OWNER_REAL_ESTATE') ?>
	                <?= Transaction_data_by_key($Row['transaction_id'],'OWNERS_MOBILE_NUMBER') ?>
	                <br>
	                طالب التقييم :<?= Transaction_data_by_key($Row['transaction_id'],'OWNER_APPLICANT_EVALUATION') ?>
	                <?= Transaction_data_by_key($Row['transaction_id'],'OWNER_MOBILE_EVALUATION') ?>
                </td>
                <td class="text-center">
